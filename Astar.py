...
def heuristic_distance(start_node, direction,qTable):
...
def AStarImplementation(start_node, goal_node,obstacles,board):
    open_nodes = {}
    ...
    while len(open_nodes)>0:
        ...
        for direction in directions:
            ...
            if(successor_node[0]<0 or successor_node[1]>=len(board[0]) or successor_node[0]>=len(board) or successor_node[1]<0 or successor_node in obstacles):
                continue
            successor_current_cost = current_cost + heuristic_distance(current_node, direction,qTable)
            print(current_cost, successor_current_cost)
            ...
            if successor_node not in open_nodes.keys() or open_nodes[successor_node][1] > successor_current_cost:
                open_nodes[successor_node] = (successor_current_cost + heuristic_distance(successor_node, direction,qTable), successor_current_cost)
...
path = AStarImplementation(start_node, goal_node,obstacles,grid)
for node in path:
